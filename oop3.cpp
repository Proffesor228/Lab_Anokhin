...
int** ummatr(int** matr1, int** matr2, size_t size)
{
	int sum = 0;
	int** umn = new int* [size];
	for (size_t i = 0; i < size; i++) umn[i] = new int[size];
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			sum = 0;
			for (int c = 0; c < size; c++) { sum += matr1[i][c] * matr2[c][j]; }
			umn[i][j] = sum;
		}
	}
	return umn;
...
void main()
{
	...
	cout << "Определитель: " << opred(matr, size) << "\nПеремноженная матрица:\n";
	int** umn = new int* [size];
	for (size_t i = 0; i < size; i++) umn[i] = new int[size];
	umn = ummatr(matr, matr, size);
	for (int j = 0; j < size; j++) {
		for (int i = 0; i < size; i++) 
		{ 
		if (umn[j][i] < 10) cout << "     "; 
		if (umn[j][i] >= 10 && umn[j][i] < 100) cout << "    "; 
		if (umn[j][i] >= 100 && umn[j][i] < 1000) cout << "   "; 
		if (umn[j][i] >= 1000 && umn[j][i] < 10000) cout << "  "; 
		if (umn[j][i] >= 10000 && umn[j][i] < 100000) cout << " ";
		cout << " " << umn[j][i];
		}
		cout << "\n";
	}
